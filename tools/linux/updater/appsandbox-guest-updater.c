...
/* Legacy migration is deliberately not a guest command anymore: the Host
 * invokes offline_legacy_bootstrap.py while the VM is stopped, rather than
 * accepting --migrate or constructing arbitrary /usr/local/libexec/%s paths
 * from guest input. */
...
typedef struct UpdateState {
    ...
    char live_backup_manifest[PATH_MAX];
    char error[160];
    uint64_t bundle_size;
    uint64_t deadline;
    int reboot_required;
 ...
 * live-file rollback journal as well as restore, so rollback never needs to
 * retain a runtime binary in memory. */
static int copy_file_atomic(const char *source, const char *target, mode_t mode)
{
    char tmp[PATH_MAX];
    struct stat st;
    ...
    uint64_t copied = 0;
    int in = -1, out = -1, ok = -1;
    if (!source || !target || stat(source, &st) < 0 || !S_ISREG(st.st_mode) ||
        st.st_size < 0 || (uint64_t)st.st_size > UPDATE_MAX_FILE_SIZE ||
        snprintf(tmp, sizeof(tmp), "%s.tmp.%ld", target, (long)getpid()) >= (int)sizeof(tmp))
        return -1;
    in = open(source, O_RDONLY | O_CLOEXEC | O_NOFOLLOW);
    if (in < 0) return -1;
    out = open(tmp, O_WRONLY | O_CREAT | O_EXCL | O_CLOEXEC | O_NOFOLLOW, mode);
...
#define STATE_STR(key, field) do { \
    ...
    STATE_STR("live_backup_manifest", s->live_backup_manifest);
...
static int write_state(const UpdateState *s)
{
    ...
    int n = snprintf(buf, sizeof(buf),
        ...
        "\"old_config_backup\":\"%s\",\"live_backup_manifest\":\"%s\",\"deadline\":%llu,"
        ...
        s->new_graphics_target, s->old_config_backup, s->live_backup_manifest,
...
static int install_release_integrations(const char *release)
{
    static const char *const units[] = {
        ...
        "appsandbox-audio.service", "appsandbox-guest-updater.service",
    ...
    {
        static const struct { const char *name; const char *target; mode_t mode; } configs[] = {
            {"50-appsandbox-gpu", "/etc/systemd/user-environment-generators/50-appsandbox-gpu", 0755},
            {"org.gnome.Shell-no-gpu.conf", "/etc/systemd/user/org.gnome.Shell@.service.d/no-gpu.conf", 0644},
            {"appsandbox-gpu", "/usr/local/bin/appsandbox-gpu", 0755}
        };
        for (i = 0; i < sizeof(configs) / sizeof(configs[0]); i++) {
            if (join_rel_path(source, sizeof(source), release, "config") < 0 ||
                append_path(prefix, sizeof(prefix), source, "/") < 0 ||
                append_path(source, sizeof(source), prefix, configs[i].name) < 0)
                return -1;
            if (access(source, R_OK) != 0) continue;
            if (strrchr(configs[i].target, '/')) {
                char parent[PATH_MAX];
                snprintf(parent, sizeof(parent), "%s", configs[i].target);
                *strrchr(parent, '/') = 0;
                if (mkdir_p(parent, 0755) < 0) return -1;
            }
            if (file_read_limited(source, &data, &len, 65536) < 0 ||
                atomic_write_file(configs[i].target, data, len, configs[i].mode) < 0) {
                free(data); return -1;
            }
            free(data); data = NULL;
        }
...
static int install_runtime_links(void)
{
    ...
    if (access(target, X_OK) == 0 && absolute_symlink(link, target) < 0) return -1;
    snprintf(link, sizeof(link), "/usr/local/libexec/appsandbox-guest-updater");
    snprintf(target, sizeof(target), UPDATE_ROOT "/current/libexec/appsandbox-guest-updater");
    if (access(target, X_OK) == 0 && absolute_symlink(link, target) < 0) return -1;
    return 0;
}
...
static void health_read_producer(HealthResult *h)
{
    static const struct { const char *name; size_t offset; } fields[] = {
        ...
        {"ready", offsetof(HealthResult, ready)}
    };
    unsigned char *data = NULL;
    size_t len = 0;
    char *line, *save = NULL;
    ...
    for (line = strtok_r((char *)data, "\n", &save); line;
        ...
        *eq++ = 0;
        if (!strcmp(line, "graphics_version")) {
            if (seen & (1U << 13) || strlen(eq) >= sizeof(h->produced_graphics_version)) goto done;
            strcpy(h->produced_graphics_version, eq); seen |= 1U << 13; continue;
        }
        ...
        if (!strcmp(line, "session_id")) {
            if (seen & (1U << 15) || value == 0 || value > LONG_MAX) goto done;
            h->session_id = (long)value; seen |= 1U << 15; continue;
        }
        if (!strcmp(line, "timestamp")) {
            if (seen & (1U << 16)) goto done;
            h->produced_timestamp = value; seen |= 1U << 16; continue;
        }
        if (!strcmp(line, "frames_encoded")) {
            if (seen & (1U << 12)) goto done;
            h->frames_encoded = value; seen |= 1U << 12; continue;
        }
...
static int health_check(const UpdateState *s)
{
    ...
    if (s->graphics_changed && h.dxg_present) {
        if (!h.producer_valid || strcmp(h.graphics_version, s->graphics_version) ||
            ...
            h.encode_failures || !h.ready || h.frames_encoded < 1) return -1;
...
static int health_main(void)
{
    ...
    printf("{\"agent_active\":%d,\"display_active\":%d,\"d3d12_helper_active\":%d,"
           ...
           "\"mismatches\":%d,\"encode_failures\":%d,\"ready\":%d,"
           "\"frames_encoded\":%llu}\n",
           ...
           h.gpu_copy, h.cpu_copy, h.cpu_conversion, h.framebuffer_mmap,
           h.cpu_memcpy_framebuffer, h.gpu_cpu_gpu, h.stale_frames, h.mismatches,
           h.encode_failures, h.ready, (unsigned long long)h.frames_encoded);
    return 0;
}

static const char *const live_transaction_paths[] = {
    UPDATE_ROOT "/current",
    UPDATE_ROOT "/previous",
    "/opt/wsl-mesa/current",
    "/opt/wsl-mesa/previous",
    "/etc/modprobe.d/asb_drm.conf",
    "/etc/systemd/system/appsandbox-agent.service",
    "/etc/systemd/system/appsandbox-display.service",
    "/etc/systemd/system/appsandbox-display-d3d12.service",
    "/etc/systemd/system/appsandbox-input.service",
    "/etc/systemd/system/appsandbox-audio.service",
    "/etc/systemd/system/appsandbox-guest-updater.service",
    "/etc/systemd/system/appsandbox-guest-update-watch.service",
...
static int live_path_allowed(const char *path)
{
    size_t i;
    for (i = 0; i < sizeof(live_transaction_paths) / sizeof(live_transaction_paths[0]); i++)
        if (!strcmp(path, live_transaction_paths[i])) return 1;
    return 0;
}

static int backup_live_files(UpdateState *s)
{
    char dir[PATH_MAX], manifest[PATH_MAX], content[65536];
    size_t used = 0, i;
    if (snprintf(dir, sizeof(dir), "%s/%s.live-files", STATE_ROOT, s->txid) >= (int)sizeof(dir) ||
        snprintf(manifest, sizeof(manifest), "%s/manifest", dir) >= (int)sizeof(manifest) ||
        mkdir_p(dir, 0700) < 0) return -1;
    for (i = 0; i < sizeof(live_transaction_paths) / sizeof(live_transaction_paths[0]); i++) {
        const char *path = live_transaction_paths[i];
        struct stat st;
        char backup[PATH_MAX], line[PATH_MAX * 2], target[PATH_MAX];
        int n;
        if (lstat(path, &st) < 0) {
            if (errno != ENOENT) return -1;
            n = snprintf(line, sizeof(line), "N\t%s\t\n", path);
        } else if (S_ISLNK(st.st_mode)) {
            ssize_t got = readlink(path, target, sizeof(target) - 1);
            if (got < 0 || got >= (ssize_t)sizeof(target) - 1) return -1;
            target[got] = 0;
            n = snprintf(line, sizeof(line), "L\t%s\t%s\n", path, target);
        } else if (S_ISREG(st.st_mode)) {
            if (snprintf(backup, sizeof(backup), "%s/%zu", dir, i) >= (int)sizeof(backup) ||
                copy_file_atomic(path, backup, st.st_mode & 07777) < 0)
                return -1;
            n = snprintf(line, sizeof(line), "F\t%s\t%s\n", path, backup);
        } else {
            return -1;
        }
        if (n < 0 || (size_t)n >= sizeof(line) || used + (size_t)n >= sizeof(content)) return -1;
        memcpy(content + used, line, (size_t)n);
        used += (size_t)n;
    }
    if (atomic_write_file(manifest, content, used, 0600) < 0 ||
        snprintf(s->live_backup_manifest, sizeof(s->live_backup_manifest), "%s", manifest) >=
            (int)sizeof(s->live_backup_manifest)) return -1;
    return 0;
}

static int restore_live_files(const UpdateState *s)
{
    unsigned char *data = NULL;
    size_t len = 0;
    char *line, *save = NULL;
    if (!s->live_backup_manifest[0] ||
        file_read_limited(s->live_backup_manifest, &data, &len, 65536) < 0) return -1;
    for (line = strtok_r((char *)data, "\n", &save); line;
         line = strtok_r(NULL, "\n", &save)) {
        char *tab1 = strchr(line, '\t');
        char *tab2;
        struct stat st;
        if (!tab1) { free(data); return -1; }
        *tab1++ = 0;
        tab2 = strchr(tab1, '\t');
        if (!tab2 || line[1] != 0 || !live_path_allowed(tab1)) { free(data); return -1; }
        *tab2++ = 0;
        if (lstat(tab1, &st) == 0) {
            if (!S_ISREG(st.st_mode) && !S_ISLNK(st.st_mode)) { free(data); return -1; }
            if (unlink(tab1) < 0) { free(data); return -1; }
        } else if (errno != ENOENT) { free(data); return -1; }
        if (line[0] == 'L') {
            if (!tab2[0] || symlink(tab2, tab1) < 0) { free(data); return -1; }
            if (fsync_parent(tab1) < 0) { free(data); return -1; }
        } else if (line[0] == 'F') {
            if (stat(tab2, &st) < 0 || !S_ISREG(st.st_mode) ||
                copy_file_atomic(tab2, tab1, st.st_mode & 07777) < 0) { free(data); return -1; }
        } else if (line[0] == 'N') {
            if (fsync_parent(tab1) < 0 && errno != ENOENT) { free(data); return -1; }
...
static int restore_activation_locked(UpdateState *s, const char *reason)
{
    ...
    if (s->runtime_changed) {
        ...
        if (runtime_owned && s->old_runtime_target[0] &&
            ...
            return -1;
        if (s->live_backup_manifest[0]) {
            /* The durable live-file journal restores every integration and
             * stable link exactly, including graphics-only transactions. */
        } else if (s->old_runtime_target[0]) {
    ...
    if (s->live_backup_manifest[0] && restore_live_files(s) < 0)
        return -1;
...
#undef MOVE_PAYLOAD_DIR
     ...
     * switch. */
    if (backup_live_files(&s) < 0 || backup_live_config(&s) < 0) goto done;
