...
static void plant_firstboot_service(ext4_writer_t *ew)
{
    ...
    const char *firstboot_sh =
        ...
        "       libasound2-dev libxcb1-dev libxcb-xfixes0-dev libdrm-dev libsystemd-dev libssl-dev \\\n"
        ...
        "        echo \"OK: make\"\n"
        "        if pkg-config --exists DirectX-Headers 2>/dev/null && [ -f /opt/appsandbox/wsl-deps/libd3d12.so ]; then\n"
        "            make -j$(nproc) d3d12-helper 2>&1 | tail -20 || echo \"WARN: D3D12 helper build failed; raw fallback remains available\"\n"
        "        fi\n"
        ...
        "    cd /\n"
        "fi\n"
        "# Seed a versioned baseline before the first signed update. The\n"
        "# independent updater must always have a previous release to restore.\n"
        "if [ -x /usr/local/bin/appsandbox-agent ]; then\n"
        "    ASB_INITIAL=/opt/appsandbox/guest/releases/initial\n"
        "    install -d \"$ASB_INITIAL/bin\" \"$ASB_INITIAL/libexec\" \\\n"
        "        \"$ASB_INITIAL/systemd\" \"$ASB_INITIAL/gnome/appsandbox-pointer@appsandbox\" \\\n"
        "        \"$ASB_INITIAL/config\"\n"
        "    for bin in appsandbox-agent appsandbox-display appsandbox-input appsandbox-audio appsandbox-clipboard; do\n"
        "        [ -x /usr/local/bin/$bin ] && install -m 0755 /usr/local/bin/$bin \"$ASB_INITIAL/bin/$bin\"\n"
        "    done\n"
        "    if [ -x /usr/local/libexec/appsandbox-display-d3d12 ]; then\n"
        "        install -m 0755 /usr/local/libexec/appsandbox-display-d3d12 \"$ASB_INITIAL/libexec/appsandbox-display-d3d12\"\n"
        "    fi\n"
        "    for unit in appsandbox-agent.service appsandbox-display.service appsandbox-display-d3d12.service appsandbox-input.service appsandbox-audio.service; do\n"
        "        [ -f /etc/systemd/system/$unit ] && install -m 0644 /etc/systemd/system/$unit \"$ASB_INITIAL/systemd/$unit\"\n"
        "    done\n"
        "    if [ -f /usr/share/gnome-shell/extensions/appsandbox-pointer@appsandbox/metadata.json ]; then\n"
        "        install -m 0644 /usr/share/gnome-shell/extensions/appsandbox-pointer@appsandbox/metadata.json \"$ASB_INITIAL/gnome/appsandbox-pointer@appsandbox/metadata.json\"\n"
        "        install -m 0644 /usr/share/gnome-shell/extensions/appsandbox-pointer@appsandbox/extension.js \"$ASB_INITIAL/gnome/appsandbox-pointer@appsandbox/extension.js\" 2>/dev/null || true\n"
        "    fi\n"
        "    if [ -f /etc/modprobe.d/asb_drm.conf ]; then\n"
        "        install -m 0644 /etc/modprobe.d/asb_drm.conf \"$ASB_INITIAL/config/asb_drm.conf\"\n"
        "    elif [ -f \"$EXTRAS/modprobe.d-asb_drm.conf\" ]; then\n"
        "        install -m 0644 \"$EXTRAS/modprobe.d-asb_drm.conf\" \"$ASB_INITIAL/config/asb_drm.conf\"\n"
        "    fi\n"
        "    # Keep the bootstrap release SemVer-valid so the first signed\n"
        "    # update can pass downgrade protection without parsing a label.\n"
        "    ASB_BASE_VERSION=0.0.0\n"
        "    if [ -s \"$EXTRAS/guest-runtime.version\" ]; then\n"
        "        ASB_CANDIDATE=$(awk -F= '/^version=/{print $2; exit}' \"$EXTRAS/guest-runtime.version\")\n"
        "        if printf '%s' \"$ASB_CANDIDATE\" | grep -Eq '^(0|[1-9][0-9]*)\\.(0|[1-9][0-9]*)\\.(0|[1-9][0-9]*)$'; then\n"
        "            ASB_BASE_VERSION=$ASB_CANDIDATE\n"
        "        fi\n"
        "    fi\n"
        "    printf 'version=%s\\ngraphics_version=0.0.0\\ncommit=provisioned\\n' \"$ASB_BASE_VERSION\" > \"$ASB_INITIAL/RELEASE\"\n"
        "    ln -sfn releases/initial /opt/appsandbox/guest/current\n"
        "    for bin in appsandbox-agent appsandbox-display appsandbox-input appsandbox-audio appsandbox-clipboard; do\n"
        "        [ -f \"$ASB_INITIAL/bin/$bin\" ] && ln -sfn \"/opt/appsandbox/guest/current/bin/$bin\" /usr/local/bin/$bin\n"
        "    done\n"
        "    [ -f \"$ASB_INITIAL/libexec/appsandbox-display-d3d12\" ] && ln -sfn /opt/appsandbox/guest/current/libexec/appsandbox-display-d3d12 /usr/local/libexec/appsandbox-display-d3d12\n"
        "    echo \"OK: seeded /opt/appsandbox/guest/current -> releases/initial\"\n"
        "fi\n"
        "\n"
        "# --- STEP 8.5: install the independent Guest Updater bootstrap ---\n"
        "echo \"==== STEP 8.5: install pinned Guest Updater ====\"\n"
        "UPDATER_BIN=\"$EXTRAS/updater/appsandbox-guest-updater\"\n"
        "UPDATER_HASH=\"$EXTRAS/updater/appsandbox-guest-updater.sha256\"\n"
        "UPDATER_KEY=\"$EXTRAS/trusted-public-key.hex\"\n"
        "if [ -x \"$UPDATER_BIN\" ] && [ -s \"$UPDATER_HASH\" ] && [ -s \"$UPDATER_KEY\" ] && \\\n"
        "   (expected=$(awk '{print $1}' \"$UPDATER_HASH\"); echo \"$expected  $UPDATER_BIN\" | sha256sum -c -) >/dev/null 2>&1; then\n"
        "    install -d /usr/local/libexec\n"
        "    install -m 0755 \"$UPDATER_BIN\" /usr/local/libexec/appsandbox-guest-updater\n"
        "    install -m 0644 \"$UPDATER_KEY\" /etc/appsandbox-guest-update-public-key.hex\n"
        "    systemctl enable appsandbox-guest-updater.service appsandbox-guest-update-watch.service 2>/dev/null || true\n"
        "    echo \"OK: pinned independent Guest Updater installed\"\n"
        "else\n"
        "    echo \"FAIL: pinned updater binary or trusted public key missing\"\n"
        "fi\n"
        "if [ -f /usr/share/gnome-shell/extensions/appsandbox-pointer@appsandbox/metadata.json ]; then\n"
        ...
        "            appsandbox-display.service appsandbox-display-d3d12.service \\\n"
        "            appsandbox-input.service appsandbox-guest-updater.service \\\n"
        ...
        "          && ln -sfn . /opt/wsl-mesa/current \\\n"
        "          && echo \"OK: wsl-mesa extracted to /opt/wsl-mesa (current -> initial)\" \\\n"
        "          || echo \"FAIL: wsl-mesa extract\"\n"
        "        echo /opt/wsl-mesa/current/lib/" IP_MULTIARCH_A " > /etc/ld.so.conf.d/wsl-mesa.conf\n"
        "        install -d /etc/vulkan/icd.d\n"
        "        if [ -f /opt/wsl-mesa/current/share/vulkan/icd.d/" IP_DZN_ICD_A " ]; then\n"
        "            ln -sf /opt/wsl-mesa/current/share/vulkan/icd.d/" IP_DZN_ICD_A " \\\n"
