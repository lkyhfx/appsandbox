...
static void plant_firstboot_service(ext4_writer_t *ew)
{
    ...
    const char *firstboot_sh =
        ...
        "echo \"==== STEP 8.5: build/install Guest Updater ====\"\n"
        "UPDATER_SRC=\"$EXTRAS/updater-src\"\n"
        "if [ -f \"$UPDATER_SRC/Makefile\" ] && command -v gcc >/dev/null 2>&1; then\n"
        "    if make -C \"$UPDATER_SRC\" -j$(nproc) 2>&1 | tail -20 && \\\n"
        "       make -C \"$UPDATER_SRC\" install PREFIX=/usr/local 2>&1 | tail -10; then\n"
        "        systemctl enable appsandbox-guest-updater.service appsandbox-guest-update-watch.service 2>/dev/null || true\n"
        "        echo \"OK: independent Guest Updater installed\"\n"
        "    else\n"
        "        echo \"FAIL: Guest Updater build/install\"\n"
        "    fi\n"
        "else\n"
        "    echo \"FAIL: updater-src missing or gcc unavailable\"\n"
        "fi\n"
        "if [ -f /usr/share/gnome-shell/extensions/appsandbox-pointer@appsandbox/metadata.json ]; then\n"
