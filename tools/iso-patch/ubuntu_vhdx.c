...
static void plant_firstboot_service(ext4_writer_t *ew)
{
     ...
     *   - No global `set -e`; the build and final binary checks use explicit
     *     gates so failure cannot reach the completion marker
     ...
     *   - The complete agent build/install transcript is saved in
     *     /var/log/appsandbox-firstboot-build.log; success shows a tail and
     *     failure dumps the complete log */
    const char *firstboot_sh =
        "#!/bin/bash\n"
        "# NOTE: explicit gates below are intentional; do not rely on set -e.\n"
        "# The complete agent build/install transcript is kept in BUILD_LOG.\n"
        "set -x\n"
        "set -o pipefail\n"
        "BUILD_LOG=/var/log/appsandbox-firstboot-build.log\n"
        "if ! install -d /var/log || ! : > \"$BUILD_LOG\"; then\n"
        "    echo \"FAIL: cannot create $BUILD_LOG\"\n"
        "    exit 1\n"
        "fi\n"
        "echo \"appsandbox firstboot build log: $(date -u 2>/dev/null || echo ?)\" >> \"$BUILD_LOG\"\n"
        "echo \"\"\n"
        "echo \"================================================================\"\n"
        ...
        "STEP8_OK=1\n"
        ": > \"$BUILD_LOG\"\n"
        "echo \"==== make agent (source=$AGENT_SRC) ====\" >> \"$BUILD_LOG\"\n"
        "if [ ! -d \"$AGENT_SRC\" ]; then\n"
        ...
        "    STEP8_OK=0\n"
        "else\n"
        "    if ! cd \"$AGENT_SRC\"; then\n"
        "        echo \"FAIL STEP 8: cannot cd to $AGENT_SRC\"\n"
        "        STEP8_OK=0\n"
        "    fi\n"
        "    echo \"---- make ----\" >> \"$BUILD_LOG\"\n"
        "    if [ \"$STEP8_OK\" -eq 1 ] && make -j$(nproc) >> \"$BUILD_LOG\" 2>&1; then\n"
        "        tail -20 \"$BUILD_LOG\"\n"
        "        echo \"OK: make\"\n"
        "        echo \"---- make install ----\" >> \"$BUILD_LOG\"\n"
        "        if make install PREFIX=/usr/local SYSTEM_UNITS= USER_UNITS= >> \"$BUILD_LOG\" 2>&1; then\n"
        "            tail -20 \"$BUILD_LOG\"\n"
        "            echo \"OK: make install\"\n"
        "        else\n"
        "            rc=$?\n"
        "            tail -20 \"$BUILD_LOG\"\n"
        "            echo \"FAIL: make install (rc=$rc); complete log follows: $BUILD_LOG\"\n"
        "            cat \"$BUILD_LOG\"\n"
        "            STEP8_OK=0\n"
        "        fi\n"
        "    else\n"
        "        rc=$?\n"
        "        tail -20 \"$BUILD_LOG\"\n"
        "        echo \"FAIL: make (rc=$rc); complete log follows: $BUILD_LOG\"\n"
        "        cat \"$BUILD_LOG\"\n"
        "        STEP8_OK=0\n"
        "    fi\n"
        "    cd /\n"
        "fi\n"
        "if [ \"$STEP8_OK\" -ne 1 ]; then\n"
        "    echo \"FAIL STEP 8: agent build/install gate failed; firstboot will not continue\"\n"
        ...
        "    fi\n"
        "done\n"
        "# Clipboard and display mode control are user-level units (graphical-session.target).\n"
        ...
        "fi\n"
        "if [ -f \"$EXTRAS/systemd/appsandbox-display-helper.service\" ]; then\n"
        "    install -d /etc/systemd/user/graphical-session.target.wants\n"
        "    install -m 0644 \"$EXTRAS/systemd/appsandbox-display-helper.service\" /etc/systemd/user/\n"
        "    ln -sf /etc/systemd/user/appsandbox-display-helper.service \\\n"
        "      /etc/systemd/user/graphical-session.target.wants/appsandbox-display-helper.service\n"
        "    echo \"OK: install user display helper unit\"\n"
        "fi\n"
        "\n"
        ...
        "printf 'GRUB_CMDLINE_LINUX_DEFAULT=\"$GRUB_CMDLINE_LINUX_DEFAULT "
            IP_EARLYCON_A IP_UART_PARAMS_A "console=tty0 console=" IP_SERIAL_A ",115200 "
            "systemd.journald.forward_to_console=1 "
            "systemd.log_target=console systemd.log_level=info loglevel=7 "
            IP_EARLYPRINTK_A " video=efifb:off video=simplefb:off\"\\n' \\\n"
        ...
        "echo \"-- final kernel cmdline --\"\n"
        "grep -h '^ *linux ' /boot/grub/grub.cfg 2>/dev/null | tail -3 || true\n"
        "echo \"-- firstboot console routing --\"\n"
        "_check_file /etc/systemd/system/appsandbox-firstboot.service\n"
        "grep -E '^(StandardOutput|StandardError)=' \\\n"
        "    /etc/systemd/system/appsandbox-firstboot.service 2>/dev/null || true\n"
        ...
        "if [ \"$CORE_BINARIES_OK\" -ne 1 ]; then\n"
        "    echo \"FAIL STEP 17: one or more core agent binaries are missing\"\n"
        "    echo \"Firstboot remains enabled; STEP 99 will not run and the VM will not reboot as installed.\"\n"
        "    echo \"Complete build log: $BUILD_LOG\"\n"
        "    cat \"$BUILD_LOG\"\n"
        "    exit 1\n"
        "fi\n"
        ...
        "if ! mkdir -p /var/lib || ! touch /var/lib/appsandbox-firstboot.done; then\n"
        "    echo \"FAIL STEP 99: cannot write completion marker\"\n"
        "    exit 1\n"
        "fi\n"
        "if ! systemctl disable appsandbox-firstboot.service 2>&1; then\n"
        "    echo \"FAIL STEP 99: cannot disable firstboot; clearing completion marker\"\n"
        "    rm -f /var/lib/appsandbox-firstboot.done\n"
        "    exit 1\n"
        "fi\n"
        "rm -f /etc/appsandbox-firstboot.marker\n"
        "sync\n"
